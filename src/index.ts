...
const GUIDANCE: PromptSection = {
  ...
  order: 150,
  text: [
    'The `houdini_*` tools drive a running SideFX Houdini session over the bridge. The executed code runs with `hou` pre-imported; `print(...)` output comes back as stdout, and assigning a JSON-serializable value to `__result__` returns structured data. Inspect the scene with houdini_query before mutating it. GUI exec failures automatically undo their Houdini-undoable scene edits and report `rollback`; file/HDA-library I/O is not undoable. In mutation execs, never swallow an exception after printing/logging it: re-raise it so the bridge reports failure and can roll back the undoable scene edits.',
    '',
    ...
    '- `render_check(path, ref=)` — luma/content stats plus high-precision A/B mean/RMSE/changed-pixel metrics. It proves image validity or pixel change, NOT visual-semantic correctness. Animation work MUST prove time dependency with `geo_frame_diff` or a meaningful fixed-camera render diff. If no vision tool successfully inspects the relayed image, report visual semantics as unverified; if node/data semantics pass but still images cannot settle subtle motion or aesthetics, hand off honestly as "visual strength pending user playback judgement" instead of looping indefinitely or claiming vision passed.',
    ...
    '',
    'Images produced by render_frame/render_view/viewport_screenshot are automatically relayed into the session workspace (`.dsh-houdini-media/`) and listed in a `media` section of the tool result — use the workspace path from that section with an available vision_* tool; the original $HIP path is NOT readable by it.',
    '',
