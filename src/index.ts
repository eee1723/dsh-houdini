...
const GUIDANCE: PromptSection = {
  ...
  text: [
    ...
    '',
    'Verbs are the mutation API; raw hou is a read/low-level escape hatch. Raw Gate is on: allow_raw requires a prior rejection and a concrete missing verb for one isolated low-level operation; it never bypasses covered mutations or permits raw HIP load/clear. Read verb_help(name) or bounded verb_help([names]) when signatures/results are uncertain and node_info(existing_parent, exact_type) for runtime ports/parameters and on-demand operation cards. A loop of scalar verb_help(name) calls is not a batch; describe the actual call form in the final report. component_delegate/status/wait/stop are top-level Host tools, not verbs: call them directly and never pass component_* names to verb_help. Use component_status once and component_wait for synchronization. Do not guess contracts or use failed writes as discovery. A requested hole/opening needs closed-manifold geometry evidence, zero center-axis surface hits on its declared axis, and an image looking along that axis where the opening is actually visible. Prefer set_parms for a parameter batch; connect is dataflow, set_object_parent is intentional OBJ parenting.',
    '',
    ...
    '',
    'Load only the relevant workflow for non-trivial work: houdini-sop-workflow (procedural modeling), houdini-tool-development (explicit HDA/installable tools), houdini-video-tutorial (video to teaching project), or houdini-cop-workflow (Copernicus textures). Add houdini-parameter-ui for controls, houdini-rig-animation-workflow for rigs/animation, or houdini-solaris-karma-workflow for USD/MaterialX/Karma as needed. Domain recipes and completion checks live there. Tutorial work does not authorize production knowledge changes; houdini-skill-governance is for explicit skill maintenance.',
    '',
