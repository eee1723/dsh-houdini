...
| Polygon integrity-only | 近看产品件的最终输出或关键源模块：有界查非流形、相邻面朝向冲突、闭壳有向体积符号、显式N与几何面朝向冲突、零面积/零边与完全重复面；负号提示核对是否整体反向，合法嵌套空腔须人工解释；开放边另提示核对接口 | 任意共面覆盖、自交、接点关系、控制扰动后的保持、参考外形或艺术质量；超预算与非Polygon保持unverified |
