...
## 13. 执行契约优先的源码修复阶段（2026-09-05）

用户授权三任务复盘后的修复与代码review。本轮先实现确定性执行合同，而非为验证而扩张评测：
strict参数失败恢复、scene_save_as、node_info、小型SOP build_module/verify_network、线程/job/
图片与HTTP边界。54动词和执行语义v2为新candidate，SOP技能增加已测接口fast path；验收与
剩余风险见 [execution-contract-review.md](execution-contract-review.md)。

这轮源码与双版本回归不等于弱模型质量增益，下一阶段顺序为：

1. Repair/restart后新session确认目录/语义版本、模块构建/失败恢复、保存和真实视觉读图；
2. 同K3同宿主同预算做旧流程/新模块路径的定向比较，先原失败，再未见同族与相邻反例；
3. 根据关系/视觉失败证据决定下一组可复用检查与证据失效状态，不直接引入完整通用状态机；
4. 达到足够增益后再按冻结规则组织v6整批，MCP adapter与模型×工具层消融仍是后续阶段。

v5协议与六场pending矩阵保持冻结；不解封holdout、不用本轮修复结果追改既有评分，不把
接口采用率或更多面数当质量。若未见任务里没有质量/稳定性/人工返工收益，就停止扩张，
收缩为轻量安全适配层。

2026-09-06 新自然会话验证已运行v2，但build_module没有采用，错误出口的verify结果未被闭环。
本轮继续收敛到v3/55：显式output、相对渲染路径与像素状态、前置结构证据、稀疏inputs、全量
有序点间距测量。双版本工具回归与原失败备份检测不等于弱模型行为成功；下一自然验收先确认
显式模块OUT检查、路径锚定和局部异常返工，再决定是否进入同K3等预算路径比较，不提前解封holdout。

2026-09-06 进一步依据test8的实际连接/无效控制反例推进v4/57候选：声明式最终表面接口+
可恢复控制case，与build_module集成。不以单纯增加工具数为目标；验证重点是“构建声明→
真实输出测量→修订→复验”。先跑双版本正反例和已有错误的只读检测，再由新K3未见任务确认
能否主动采用并修好结果。控制测试的Packed/NURBS/volume表示暂未支持，零写入unverified；
不可用放宽完成门掩盖未知。视觉细化、稳定构造库和全局证据状态仍为后续阶段。
