...
- 状态：待验证
- 现状：同层模块、稳定输出、Y-up、局部接口和表面快检已有机制。公开水瓶题自然采用VEX剖面→Revolve/PolyWire，早期负向/重合风险被修，保存HIP独立重开后外壁、内壁、封底与四个闭壳方向正确；但无真实螺纹，提环承力连接未证，密封零距离被说成压紧。原`test_controls`把全局合盖接触合同套到开盖状态，模型为取数值5/5删去关系检查；现已加case专属interfaces并在H21/H22隔离验证，live自然采用待验。近景独立截面已核，独立阴影视图仍缺；单题改善不证明泛化。
- 下一步：在不同可开合/脱离产品题验case专属接触与分离合同的自然采用，机械连接和中间姿态须与外形、控制响应分开验收；对未完成的核心结构不能以声明简化直接停止。另补未见Y-up装配、公共Output/空Pack/HDA和H21复杂PolyBevel GUI反例；布局与连接件归属按真实网络验证。局部pass不外推全局。
...
- 状态：待验证
- 现状：原始材料、澄清和来源回读已有；Trace的retryWork.hostWork补充同文件写入、相同shell命令及无Houdini调用区间，弥补离线计算循环不可见。公开水瓶题在约17/40分钟结束时自认缺少旋盖螺纹却报告“已做完”，形成可复核的提前停止反例；不把一次停工直接归因于上下文压缩或模型时限。完整义务/依赖计划、摘要压缩和中途纠正仍缺受控行为证据。
