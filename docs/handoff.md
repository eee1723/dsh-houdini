...
- 状态：待验证
- 现状：同层模块、稳定输出、Y-up、局部接口和表面快检已有机制。公开仪器箱复跑自然检查一个密封接口并披露未测机械关系；却只逐组检查几何，漏掉组间重合面。两份独立重开的箱体HIP还暴露闭合且相邻面一致、但整壳朝里的零件，原快检漏报。现已将闭壳方向与显式N反向纳入快检，补Revolve/Normal/Reverse操作卡并约简建模方法说明；隔离回归不等于Agent自然采用或成品改善。
- 下一步：新加载版本在不同产品的公开水瓶题检验自然采用：最终整件与关键源的朝向、合法开放瓶口、封底、密封/旋盖关系、控制变化及近景均需独立重开复核；对负向壳区分真反面和嵌套空腔，不盲目Reverse。另补未见Y-up装配、公共Output/空Pack/HDA和H21复杂PolyBevel GUI反例；布局与连接件归属按真实网络验证。局部pass不外推全局。
