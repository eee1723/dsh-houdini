...
- 下一步：新加载合同用未见Y-up装配任务验世界轴、稳定输出、组件/角色两层布局、连接件归属和相对依赖；仅在端口稳定、独立复用/替换/组件作者或用户明确层级时采用Subnet。近看Polygon末端检查仍是候选：新session用未见同族模型和有意开放端口反例验自然调用、异常归因与修复后复验，不能把规则文字当能力提升。继续验公共Output、空Pack、HDA实例及“要求→控制→关系→恢复→复验”；继续约简原复杂工程的PolyBevel附加崩溃条件。局部pass不外推全局。
