...
- 状态：待验证
- 现状：同层模块、稳定输出、Y-up、局部接口和整件表面快检已有机制；GA字符串属性名称表重排导致的恢复误报已修。另一件开合产品的自然任务主动使用了baseline/case接口，保存HIP独立重开后铰链和前缘三态关系成立，但开盖分离仅由评审独立补测，Agent正式测试只覆盖销轴和bbox。更严重的是Agent口头把1场景单位当1厘米，实际HIP单位仍为1米，尺寸错100倍且最终报完成。已在scene_info和一次性现场摘要中暴露真实单位长度，并在SOP工作流要求尺寸换算；合同64的live采用尚未验，旧运行版本也无精确health证据。局部造型仍偏简单，单题改善不外推泛化。
- 下一步：新加载版本用不同物理尺寸产品验证Agent是否读取真实场景单位、按用户单位建模并在交付前复核；对活动产品让Agent自己逐状态声明基准贴合和扰动分离，而非只测整体bbox。另补未见Y-up装配、公共Output/空Pack/HDA和H21复杂PolyBevel GUI反例；机械连接、局部近景与外形/控制响应分别验收，只把明文或用户确认的结构设为强制完成门。
...
- 状态：待验证
- 现状：原始材料、澄清和来源回读已有；Trace的retryWork.hostWork补充同文件写入、相同shell命令及无Houdini调用区间，弥补离线计算循环不可见。公开水瓶题未明写真实螺纹，40分钟仅是未送入Agent的外部评审上限，因此不能用本轮“17/40分钟”当提前停工证据；提环明文功能的验证缺口与密封过述仍需独立处理。完整义务/依赖计划、摘要压缩和中途纠正仍缺受控行为证据。
- 下一步：新session对明文或用户确认的核心要求验证停止规则候选：可安全推进时继续，部分报告/保存不作停止许可；真实限制保留身份/未完成项/下一步，可选美化不无限续跑。系统上下文剩余量与自动续接尚未实现，不以模型自述预算充当系统证据。若测预算规划，须把同一时间上限明示给可比试跑的Agent；用短/信息完整/冗长等价任务加中途变更和压缩续跑作对照，收费模型另授权，不加默认长问卷或第二份需求账本。
