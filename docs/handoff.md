...
- 状态：待验证
- 现状：同层模块、稳定输出、Y-up、局部接口和表面快检已有机制。公开水瓶题自然采用VEX剖面→Revolve/PolyWire，早期负向/重合风险被修，保存HIP独立重开后外壁、内壁、封底与四个闭壳方向正确；提环“可提起”的连接未证，密封零距离被说成压紧。螺纹不是该公开题明文或评审硬要求，不能据此判违规停工。原`test_controls`把全局合盖接触合同套到开盖状态，模型为取数值5/5删去关系检查；现已分别支持只验基准的baseline_interfaces和只验对应扰动的case.interfaces，H21/H22隔离验证通过，live自然采用待验。近景独立截面已核，独立阴影视图仍缺；单题改善不证明泛化。
- 下一步：在不同可开合/脱离产品题验合盖基准接触与开盖分离合同的自然采用，机械连接和中间姿态须与外形、控制响应分开验收；只把明文或用户确认的结构作为强制完成门。另补未见Y-up装配、公共Output/空Pack/HDA和H21复杂PolyBevel GUI反例；布局与连接件归属按真实网络验证。局部pass不外推全局。
