...
- 下一步：在明确授权的输入/依赖合同下验证现有场景到隔离worker的移交，不自动复制live HIP或重新认领节点；补live协作取消并验证原生中断，在新加载运行态验收Manual/新鲜度边界。实现节点UUID+工程登记恢复，不能仅凭tag授权。图片恢复需检查Host请求waterfall与冻结请求合同，保留原始历史，不直接改node_modules。继续验证断联回执/过期/runtime更换。
