...
| P0 / 部署主线 | H-01 部署更新与运行态验收 | 先锁发布边界，再做引导/受管环境/旁路更新；H21/H22实际路径仍需授权 |
...
- 状态：待验证
- 现状：签名离线包、独立引导/管理器、受管依赖/profile/端口和版本/数据快照回退已有；正式公钥release-key-1已配置，私钥由维护者在仓库外保管，尚欠备份确认和发布环境，隔离测试不证明用户live已加载。
- 下一步：以0.0.1为首发目标，确认私钥安全备份，准备受保护release环境/H21/H22 runner并启用immutable Releases；在干净机器和获授权H21/H22用户路径验收安装/更新/回退与实际加载身份后发布。继续核对启动/重连/失败回包、来源/工作流、新session指代/提醒/压缩恢复、原生/Code Mode图像及Trace历史；WebView须完整重开，不以测试包或草稿核销live缺口。
- 移除条件：安装/损坏修复/断网/更新失败保旧/重启/回退和H21/H22真实用户路径均有证据；main push不可见，正式版无用户build，配置/会话及HIP不丢失；隔离回归不得冒充live验收。
