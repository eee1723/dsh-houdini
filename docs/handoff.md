...
- 下一步：本仓库的源码分支、构建产物或官方0.1.5-rc.2不能替代该候选；升级基线是独立资格重验，不随普通开发rebase。用新加载skill复验未见两组件任务：父作者必须先读协作reference，默认普通subnet→component_export→hash/revision→component_import→槽位接纳，不得自行升级HDA或裸安装；明确HDA交付任务作为反例。集成输出须在同一坐标系实测关系，跨OBJ代理显式保留world变换，proxy bbox与源world bbox一致且warning-free后才可读图。继续验异常worker展示、局部图、公共控制、迁移表达式/keys、修订失效和GUI双worker渲染互斥。C5以固定模型/版本/信息/质量门比较单作者与多作者的主子总请求、首次正确组件、HDA/打包尾段、失败/rollback和最终缺陷，不用并行数量或单一墙钟证明收益。
