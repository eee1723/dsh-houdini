...
assert.match(gui,/ignore every parent-supplied export path.*\$HIP\/<name>\.dshcomponent/s)
assert.match(gui,/Report the returned absolute filename, sha256 and contract/)
assert.match(gui,/scene_save\(\).*never Save As/)
assert.match(gui,/render_view.*inside houdini_exec\/query/)
assert.match(gui,/component_delegate\/status\/wait\/stop are unavailable/)
assert.match(gui,/verb_help documents only these verbs/)
...
assert.match(gui,/node_info\(part,'circle',parm_filter='radius'\)/)
