...
try:
    ...
    config = {'python': sys.executable, 'houdini': hython, 'workerRoot': str(configured_workers), 'executorRegistry': str(registry),
    ...
    while not (outputs / ('rejected.json' if negative else 'assembled.json')).exists():
