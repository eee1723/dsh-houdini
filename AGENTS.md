...
跨能力族 discovery 已完成，但因模型替换、额度中止和 evaluator 输入未完全冻结，只作为发现证据。B0 smoke/brief/answers/seed/evaluation schema、真实产物与跨文件 hash validator 已实现；通用 seed generator 已支持空场景和固定 shaderball，三族 calibration seed 输入已用 H21 重复生成并通过结构 identity/真实 HIP hash 校验，H21/H22 HOM 回归通过。执行模型已选 `kimi-coding/k3` 与 `apikeyfun/glm-5.3-flash`；独立 evaluator 固定为 DashScope `qwen-vl-max`。三族隔离 smoke 已通过。Protocol `b0-2026-09-02-v4` 的 normalizer 要求 criterion/image/deterministic ID 合同，确定性生成 hard failure 并校验 critical aggregate。正式 3×2 已完成 2/6：Mechanical/K3 与 Simulation/GLM 均为 100 分、0 hard failure、claimLevel=none；GLM 项约 74.9 分钟、14 failed calls/11 rollback，明显比 K3 机械项更慢，但跨任务尚不可排名。下一顺位：Lookdev/K3。生产面不得写 benchmark 实例答案；holdout 仍未解封；`ctx.jobs`、approval 层和大规模模块拆分继续放在整批之后。`houdini_query`/`houdini_exec` 暂时保留为两个工具，是否合并只按正式运行的误选率、额外调用与安全收益证据评估。
