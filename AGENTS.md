...
2026-09-04 新增 `houdini-codex` 个人/换机可复现 preset：profile requirements 固定安装 `@deepseek-ai/dsh-subagent-codex@0.1.2-rc.1`（包内官方 `@openai/codex@0.149.1` app-server），launcher 自动把 `presets/houdini-codex/` 同步到用户 preset 根。父模型禁用 shell/fs/web/普通 subagent/workflow，实质性任务首调 `subagent_codex`；live Houdini 仍按 Codex 决策由父级 `houdini_*` 执行并二次交 Codex 验收，因为当前 provider 不转发宿主工具。真实 ChatGPT 登录 smoke 的首个工具调用与结果 `CODEX_SUBSCRIPTION_OK_7F3A` 已通过。该约束是 persona + 工具裁剪，不冒充 agent-loop 硬 `tool_choice`；父 provider 仍需少量额度。Git/安装器不复制 `~/.codex` 凭据，新机器必须官方 `codex login`。当前机器无可用 Houdini executable/process，新增 profile composition 的 H21/H22 QtWebEngine smoke 尚未重跑，baseline 明确 `matchesBaseline=false` 并保留 pendingReload，完成前不得把 Host smoke 扩写为完整 GUI 兼容发布。
