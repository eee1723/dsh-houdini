...
跨能力族 discovery 已完成，但因模型替换、额度中止和 evaluator 输入未完全冻结，只作为发现证据。B0 基础设施、三族 seed/smoke 与 Protocol `b0-2026-09-02-v4` 已冻结；normalizer 校验 criterion/image/deterministic ID，确定性生成 hard failure 与 critical aggregate。正式 3×2 已完成 3/6：Mechanical/K3=100，Simulation/GLM=100，Lookdev/K3=90；三项均 0 hard failure、coreSuccess=true、claimLevel=none。Lookdev/K3 因报告声称 Dome Light 但最终 stage 无 DomeLight prim，honest-report 扣 10 分；GLM Simulation 的运行时间/失败/回滚显著较高。下一顺位：Mechanical/GLM；尚不可给出模型排名。生产面不得写 benchmark 实例答案；holdout 仍未解封；`ctx.jobs`、approval 层和大规模模块拆分继续放在整批之后。`houdini_query`/`houdini_exec` 暂时保留为两个工具，是否合并只按正式运行的误选率、额外调用与安全收益证据评估。
