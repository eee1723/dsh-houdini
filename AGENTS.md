...
跨能力族 discovery 已完成，但因模型替换、额度中止和 evaluator 输入未完全冻结，只作为发现证据。B0 smoke/brief/answers/seed/evaluation schema、真实产物与跨文件 hash validator 已实现；通用 seed generator 已支持空场景和固定 shaderball，三族 calibration seed 输入已用 H21 重复生成并通过结构 identity/真实 HIP hash 校验，H21/H22 HOM 回归通过。执行模型已选 `kimi-coding/k3` 与 `apikeyfun/glm-5.3-flash`；独立 evaluator 固定为 DashScope `qwen-vl-max`。Mechanical attempt 1 因 repo workspace 污染 invalidated；attempt 2 与 Simulation attempt 1 均在隔离 workspace 完成，HIP/cache/render/trace/final node/Blind/Target delivery 通过，smoke 不评分。Protocol `b0-2026-09-01-v3` 的严格 JSON normalizer 已实测覆盖 raw JSON 和单层 JSON fence。Simulation trace 另暴露 File Cache 写盘按钮的目录动词缺口：5 次无豁免调用被 Gate 阻止，随后两轮写盘使用同一明确 `allow_raw` 理由；query 无 mutation。下一顺位：按同一隔离流程运行 Lookdev smoke，通过后再启动正式 3×2。生产面不得写 benchmark 实例答案；`ctx.jobs`、approval 层和大规模模块拆分继续放在证据阶段之后。`houdini_query`/`houdini_exec` 暂时保留为两个工具，是否合并只按后续误选率、额外调用与安全收益证据评估。
