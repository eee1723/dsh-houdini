...
SOP公共端口用sop_set_output(末端,output_index=端口索引)复用/创建并接线，再用verify_network(parent,output=末端,output_index=同索引)验收。
仅在subnet/HDA边界发布，不自动猜根输出；普通geo不要求Output节点，明确最终SOP及display/render即可。Geometry类型HDA还要把根层最终SOP显示设好。内部Object Merge/表达式用实例相对引用，
不得硬编码某个源实例路径；在不同名称/位置创建两个实例，各改一个控制，确认互不串读。外置材质也是交付依赖。
...
先分清修改对象：实例普通参数用set_parms；内部网络在明确实例中通过节点/连线动词修改；定义界面用hda_set_interface；PythonModule/回调先读section，必要时在Host编辑并检查源码，再用hda_set_section/hda_patch_section写回。package外置模块在其唯一源码处修改，独立验证后再按授权加载。不要为一处section修改重建整个HDA，也不直接解压/改写.hda二进制。

维护已有库前确认实际定义文件、同类型其他实例及package归属。未经明确原库替换授权，先在独立工作副本中验证，不自动覆盖共享库或安装目录；副本若保留同类型名也可能抢占当前定义，所以先在隔离进程加载，不能把复制文件当作live隔离。新实例/副本不自动继承原节点ownership。
