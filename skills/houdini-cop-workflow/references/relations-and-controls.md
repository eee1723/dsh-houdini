...
先对照单元轮廓/边缘、相对高度与图案尺度，再看组合后的遮罩分布和真实材质消费；最终图不符合时，
用中间图层定位最早分叉。整体远景不能证明局部损伤，ID最大值不能推出唯一数量、行列布局或UV跨度，
这些分别从实际ID集合、空间分布和消费端坐标取证。缺失参数按video复现协议试验，不把设参成功当效果正确。
阈值/遮罩驱动材质时，分别量测被选区域、未选区域及边界的输出；全图均值变化不能证明区域对应正确。
归一化遮罩的量程受上游极值影响：改了高度/噪声后，旧重映射阈值不自动维持相同覆盖与过渡。
先查覆盖和区域内响应，再调整映射；不固化某个任务的阈值、覆盖比例或“只要非零”的验收值。
