...
- 单个SOP产品的控制默认放在该产品的Geo内部（如`/obj/product/CTRL`），让参数和可编辑网络一起交付；先建Geo，再在其中建Null或选已有合适SOP载体。跨多个OBJ的真实场景总控才放`/obj`层，并说明所控对象。create_spare_parms(layout=...)复用组件，默认只追加，拒绝同名覆盖；已有默认值修改走update_defaults。
