...
2. **本地准备与概览**：先用 `scan` 粗截图认识章节和界面；它不依赖音频或云服务，可与音频准备分别推进。
...
5. **索引与定向取证**：转录到达后，用 `index-init` 建资料索引，按 `read-transcript` 分页读原文；
...
6. **音画对照与保留**：用 `context` 将选定区间的原图与已有转录组成 hash 绑定的证据包，不重发 ASR。
   ...
   重复画面用状态快照加变化记录，保留试调、撤销与后续修正。索引只引用 notes，不复制另一份观察事实。
