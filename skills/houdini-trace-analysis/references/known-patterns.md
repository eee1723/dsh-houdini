...
- 坐标/交接复核：`dsh-houdini-f05cde129f7540b78ee98b55bc996a49` 的可调装配任务建立模块输出、
  稳定`OUT_ASSET`、关系探针和控制扰动，但把X/Y/Z解释为宽/深/高并以`min Z=0`自证接地；Houdini
  默认Y-up视口中资产侧躺。自动取景图没有地面参照，组件大框又吞并了source/placement/Copy/output
  角色。该中文“程序化+关系保持+网络清楚”请求曾被提取器误判`applicable=false`；现已补词法回归，
  SOP完成门要求新资产原生Y-up、旧工程根输出前适配、逐足世界接地及一层组件容器→角色小框；受治理工具已用H21/H22 GUI复核父子框、分层布局、undo与保存重开，但自然任务采用仍待新任务。
...
- 下一验收：再用一个未见Y-up装配任务确认默认世界轴、根输出、地面参照和组件/角色布局能自然采用；
  体积/模拟任务继续检查简化是否在 mutation 前披露，并为承诺形态提供独立于整体hero图的数值或分层诊断。
