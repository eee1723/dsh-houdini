...
6. 视觉工具 transport 成功不等于看图成功。bootstrap 是 setup、present 是交付；显式视觉工具只有成功的 semantic
   inspection 可作该工具的视觉结论。原生附件用 `nativeImages[].followingAssistant.seq` 定位后续模型描述，
   人工逐图对照原始图片、观察问题及最终声称后才判其证据范围；模型自述、附件送达和脚本关联本身均不认证语义正确。
   结构化 `ok:false`、模型声明不支持图像/图片被省略等文本拒绝必须判失败。
...
- `cacheReadTokens`是每次请求的命中量，不要求单调；缺值保持未知，不当作0。相邻回退或前缀替换的时间对应只给关联线索，不能把差值相加直接命名为额外计费/浪费token或缓存失效根因；性能因果需同模型、同信息与相同工作负载的请求级对照。
