...
模块合同不等于Subnet合同，不因模块化自动创建Subnet。普通单作者任务默认可用同层逻辑模块：源、局部处理、放置/复制与稳定`OUT_<MODULE>`保持相邻，由Network Box辅助阅读；根装配只接这些检查点。只有用户明确要求层级，或模块公共端口稳定且需要独立复用、导出、替换、组件作者、或根网络已明显难以接手时，才提升为普通SOP Subnet。连续曲面、跨模块Boolean、强耦合求解、短链或接口频繁变化不强拆。容器变化不改变原模块义务，也不自动升级为HDA。

私有细节由叶模块负责；同时依赖两个模块真实表面/安装frame的螺栓、销钉、焊缝等连接件归最低共同装配层。任何模块不得通过绝对OBJ/工作区路径偷读兄弟内部节点。共享尺寸只有一个维护源；同层优先相对兄弟引用，Subnet使用公共输入/参数和内部相对引用。局部参数一旦改变外包络、接口或邻接间隙，就不再是私有实现。
...
停止条件由persona统一维护：原始必需项有安全可执行下一步时继续；真实资源/权限阻塞时保留具体证据与续接动作。
不要用主观“预算不足”跳过模块验收，也不把额外可选精雕当作永远不能结束的理由。

集成先由需求与当前控制状态导出期望成员及基数，再确认这些声明部件在实际最终输出中非空且基数一致，最后复验变换后的关系。不能以最终输出里幸存的组反向定义期望集合。源模块可cook、
build_module.required_outputs通过，只证明源分支存在，不能证明下游Switch/选择/Merge还包含它。
代理体与正式模块保留可区分身份；替换代理后复核最终成员，不能让代理继续填补正式部件的缺失。
Packed外壳非空不证明内容非空或部件齐全；verify_network只验证有实际内容，成员/基数仍从交付几何另查。
模块接口同时声明local/world空间、up/forward轴和rest/current状态；正式新建的普通Houdini交付从首个源/骨架开始使用世界Y-up（X宽、Y高、Z深），不把根旋转当默认建模步骤。复制只接受约定的局部原型，世界空间附件不能再次套实例变换。旧工程或确有独立局部frame的模块使用Z-up时，在父装配的稳定`OUT_ASSET`之前只做一次显式local→world适配，并在适配后的真实输出上重测世界包络和接地；`OUT_ASSET`之后的Transform旁路不是修复。
共享运动控制的附件必须消费同一当前状态，静止位置公式通过不能替代运动后的连接。

观察色只承担接手和视觉分辨：优先由稳定`part`/piece身份在装配末端派生`Cd`，同组件一致、不同组件可辨认；不要把节点路径、primitive序号或拓扑偶然顺序当颜色身份。若输入已有用户颜色、材质或下游lookdev合同，默认保留并改用隔离预览/分组观察。`Cd`存在或画面颜色不同不证明部件存在、关系正确或材质交付完成。

后续修改先回读目标：接口/共享控制变更使相关邻接检查失效；计数、组、拓扑或输出身份变化还会使旧成员范围和接口合同失效。证据需绑定输出身份、frame、相关参数及可用指纹；内部细节变化至少刷新本模块及实际受影响的集成证据。
...
共享控制联动/恢复以及独立已提交模块保留；`tools/tests/dsh-output-publication.test.py`覆盖公共输出/内部显示分离、
多端口、foreign拒绝与空Packed内容。它们不运行LLM，不能证明自然任务的模块聚焦行为或美术质量提升。
...
观察→参数扰动→关系复查→完整恢复应在test_controls内完成，不能以恢复了几个bbox替代bgeo恢复证据。顶层`interfaces`是基准和每个case共同成立的静态合同；每个case可另带`interfaces`，只检查该扰动状态，适合接触变分离等状态转换。二者都是明确列表，不执行回调或自动猜成员；若计数变化后无法为新状态声明完整选择和基数，就拆成有明确范围的受支持检查，未覆盖部分保留unverified，不能删expected_points或只测旧固定子集。
