...
容器是可选实现，模块合同是必需结构。普通单作者建模默认先在同一SOP网络建立平铺逻辑模块：源、局部处理、放置/复制、组装与稳定检查点沿数据流相邻；一种组件类型只有一份recipe，重复实例Copy其输出，不为每个实例复制完整网络。父装配只消费`OUT_<MODULE>`，非平凡根资产发布`OUT_ASSET`。Network Box改善同层可读性但不是层级或接口。

满足独立复用/导出/替换、组件作者、用户明确层级、少量稳定公共端口或根网络明显难以接手时，才把已验证的平铺模块提升为普通SOP Subnet。提升使用候选分支：记录原输入/消费者/参数依赖，发布原生Output并独立复验后再切消费者，不原地折叠和删除旧网络。强耦合连续曲面、跨模块Boolean、接口频繁变化或短链保持同层。HDA只由明确共享类型/安装分发意图触发。
